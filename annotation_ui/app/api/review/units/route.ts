...
import {
  getReviewUnits,
  getCompletedReviewsForReviewer,
  getLocalCompletedReviews,
  isS3Configured,
} from "@/lib/data";
...
export async function GET(req: NextRequest) {
  ...
  if (reviewer) {
    try {
      completed = isS3Configured()
        ? await getCompletedReviewsForReviewer(reviewer)
        : getLocalCompletedReviews(reviewer);
    } catch (err) {
